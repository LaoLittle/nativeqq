...
#   endif
    ...
    TC_PackIn& TC_PackIn::write(const char *sBuffer, size_t length) {
        getBuffer().append(sBuffer, length);
        return *this;
    }
...
/************************************************************************/
    ...
    bool TC_PackOut::skip(size_t length) {
        if(isEnd() && _pos + length >= _length)
        {
            return false;
        }
        _pos += length;
